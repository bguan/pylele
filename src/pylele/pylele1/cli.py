...
from pylele.pylele1.config import (
    ...
    DEFAULT_LABEL_SIZE,
    DEFAULT_LABEL_FONT,
    DEFAULT_LABEL_SIZE_BIG,
    DEFAULT_LABEL_SIZE_SMALL,
    ModelLabel,
    TunerType,
)
...
def parseCLI():
    ...
    parser.add_argument(
        "-s",
        "--scale_length",
        help=f"Scale Length [mm], or {LeleScaleEnum.list()}, "
        + "default: {LeleScaleEnum.SOPRANO.value}",
        type=LeleScaleEnum.type,
        default=LeleScaleEnum.SOPRANO,
    )
    parser.add_argument(
        "-n",
        "--num_strings",
        help="Number of strings, default 4",
        type=int,
        default=4,
    )
    parser.add_argument(
        "-a",
        "--action",
        help="Strings action [mm], default 2",
        type=float,
        default=2,
    )
    parser.add_argument(
        "-g",
        "--nut_string_gap",
        help="Strings gap at nut [mm], default 9",
        type=float,
        default=9,
    )
    parser.add_argument(
        "-e",
        "--end_flat_width",
        help="Flat width at tail end [mm], default 0",
        type=float,
        default=0,
    )
    ...
    parser.add_argument(
        "-w",
        "--wall_thickness",
        help="Chamber Wall Thickness [mm], default 4",
        type=float,
        default=4,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-t",
        "--tuner_type",
        help=f"Type of tuners, default; {TunerType.FRICTION.name}",
        type=str.upper,
        choices=TunerType.list(),
        default=TunerType.FRICTION.name,
    )

    parser.add_argument(
        "-d",
        "--dot_frets",
        help="Comma-separated fret[:dots] pairs, "
        + "default 3,5:2,7,10,12:3,15,17:2,19,22",
        type=lambda d: {
    ...
    parser.add_argument(
        ...
        + "default Pylele:24:Verdana,:6,'mind2form.com © 2024':8:Verdana",
        type=lambda x: [
